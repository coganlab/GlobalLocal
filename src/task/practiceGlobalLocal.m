function [accuracy, exited] = practiceGlobalLocal(windowPtr, fontDefault, maxWidth, maxHeight, ...
    ...
    while trial <= nTrials
        ...
        if paused 
            ...
            if paused
                ...
                writetable(out_data,strcat(subjectDir,'/GL_GlobalLocal_Practice_Data_#',num2str(iteration),'_',subID,'_',time,'.csv'));
...
function taskArr = createTaskArr(nTrials)
    taskA = 'g';
    taskB = 'l';
    ...
    %get trial numbers
    nSwitchTrials = floor(nTrials * percSwitch);
    nRepeatTrials = nTrials - nSwitchTrials;
