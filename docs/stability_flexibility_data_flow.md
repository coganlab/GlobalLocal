...
| `docs/stability_flexibility_outputs_guide.md` | *What does each output PNG/CSV/JSON/TXT mean, and why are counts sometimes sparse?* |
...
python docs/examples/stability_flexibility_data_flow_demo.py           # all stages, ~5 min
...
cluster, no BIDS tree, and no iEEG data on disk** — every stage uses the module's
own synthetic generator with *planted ground truth*, so you can always check the
answer against what was planted. Every output below is pasted verbatim from that
script; it is seeded, so you will get the same numbers.

The one exception is the decode inside `a4`, which goes through the project
`Decoder` and therefore needs `ieeg` (cluster environment). Without it that stage
still prints the whole data flow — the arrays the decoder would consume — and
skips only the final ground-truth check.
...
**A4 is not a separate pipeline.** It runs on the ordinary decoding stack, which
already provides everything the transfer needs — the ROI LabeledArray is a
cross-subject pseudopopulation (subjects NaN-padded and concatenated along the
channel axis), the CV split makes train and test trials disjoint, `shuffle=True`
gives a refit null, and `time_perm_cluster` corrects across windows. The only new
ingredient is a **second label vector**.

```
input is the ordinary ROI LabeledArray — the SAME structure the normal
decoding path consumes: {roi: {condition: (trials, channels, time)}}
  16 conditions, e.g. ['Stimulus_c_r_25inc_25sw', 'Stimulus_c_r_25inc_75sw']
  each: (40, 40, 32)

build_cross_decoding_arrays -> TWO labellings of the SAME trials:
  data          (640, 40, 32)
  labels_train  [1 1 1 1 ...]  (congruency, cats={('_i_',): 0, ('_c_',): 1})
  labels_test   [1 1 1 1 ...]  (switchType, cats={('_s_',): 0, ('_r_',): 1})
  strata        [0 0 0 0 ...]  (16 source conditions)
  joint (train, test) cells: [(0,0), (0,1), (1,0), (1,1)] -> counts [160, 160, 160, 160]  <-- the contrasts CROSS
```

Two things to notice:

- **The contrasts must cross.** All four `(train, test)` label combinations have
  to be populated, or a transfer isn't identifiable — you'd be asking whether an
  axis generalizes to a labelling that is a copy of the one you trained on.
  `build_cross_decoding_arrays` drops any condition that only one contrast can
  label, and raises if nothing survives.
- **`strata` is the source condition, and it is what you stratify on.** A split
  balanced only on the *training* labels can hand you a test fold that is
  lopsided on — or entirely missing — a class of the contrast you **score**.

Then it's an ordinary decode with one extra argument:
...
```
  shared     within congruency  true = 0.753   shuffle = 0.513
  shared     cross stab->flex   true = 0.716   shuffle = 0.520
  orthogonal within congruency  true = 0.911   shuffle = 0.509
  orthogonal cross stab->flex   true = 0.494   shuffle = 0.502
```

Both synthetic worlds are *individually* decodable — `orthogonal` is in fact the
easier within-contrast decode at `0.911`. They differ only in whether the two
contrasts load on the same population axis, and the transfer accuracy separates
them cleanly: `0.716` vs `0.494`. Chance here is the **refit shuffle null**
(train labels permuted, whole pipeline re-run), not an assumed 0.5.

### The train/test proportion

`frac_train` sets the proportion directly, using `StratifiedShuffleSplit` instead
of `StratifiedKFold`:

```python
decoder.cv_cm_jim_window_shuffle(..., frac_train=0.5)   # 50/50 instead of (k-1)/k
```

`n_splits` then means "random resamples per repeat" rather than "folds", and the
resamples are no longer a partition. Useful for asking how much of an effect is
training-set size — expect accuracy to fall as `frac_train` shrinks.

### Temporal generalization

`temporal_generalization=True` fits at each train window and predicts at every
test window, returning `(n_train_windows, n_test_windows, n_samples, n_cats,
n_cats)`. Broad off-diagonal generalization → a sustained code; a narrow diagonal
→ a moving/phasic one. Cost is `n_windows` fits and `n_windows²` predictions, so
start with a coarse `step_size`. Its diagonal is exactly the ordinary windowed
run (asserted in `tests/analysis/decoding/test_cross_decoding.py`).
...
the 2×2, and it must be ignored, **or** run under the disjoint trial split (§8).
Cross-validation alone does not rescue it: the selection happened *before* the CV
split, using every trial, so every fold is contaminated. The **off-diagonal**
cells are the ones CV makes honest.

### The within-block 2×2 (Fig 9)

"Decode a contrast within one block level" is an ordinary decode over that
block's conditions — no special code path:

```
filter_conditions(..., "25inc") -> 8 of 16 conditions
```

then run with the same contrast for train and test. Comparing the two block
levels' accuracy traces is the decoding analogue of the univariate LWPC / LWPS
effect.
