...
def main(subject_id):
    ...
    for condition_name, event_list in events_to_count.items():
        event_name_to_find = events_list[0]

        count = np.sum(good_data.annotations.description == event_name_to_find)
            
        trial_counts[condition_name] = count
        print(f"Found {count} trials for {condition_name} ('{event_name_to_find}')")

    #edit to save to right pathway
    save_dir = os.path.join(os.path.dirname(os.path.abspath(__file__))
    os.makedirs(save_dir, exist_ok=True)
    counts_filename = os.path.join(save_dir, f'{subject_id}_counts.csv')
